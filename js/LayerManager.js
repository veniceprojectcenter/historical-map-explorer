			...
			dataService.getFeaturesForLayer(type, function(feature) {
				...
				if (feature.properties.type == type && feature.geometry) {
					...
					newPoly.on('click', function() {
						...
						//if (numMaps > 0) {
							/*content += '<div><summary>This feature appears on '+numMaps+' other '+(numMaps === 1 ? 'map' : 'maps')+'</summary>';
						...
						//	console.log("Mappe", mapManager, maps, feature.properties.maps);
							Object.keys(maps).map(function(k) {
								// content += '<p map_id="'+k+'">' + maps[k].name + '</p>';
								if (k === currentMap) return;
								if (feature.properties.maps.indexOf(k) >= 0) {
									content += '<a class="show_other_map" data-map-id="'+k+'">'+mapManager.mapLabel(k)+'</a>';
								} else {
									content += '<p class="map_not_linked">'+mapManager.mapLabel(k)+'</p>';
								}
							});
							content += '</div>';
						//} else {
